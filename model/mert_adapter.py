    ...
    def __init__(
        ...
        if ratio == 1:
            ...
            self.k_proj = nn.Sequential(
                ...
                nn.Linear(embed_dim, embed_dim//ratio, bias=bias),
                nn.ReLU(),
                nn.Linear(embed_dim//ratio, embed_dim, bias=bias)
            )
...
class HubertEncoderLayer(nn.Module):
    def __init__(self, config, adapter_layer_index):
        super().__init__()
        self.adapter_layer_index = adapter_layer_index
        ...
        if adapter_layer_index == 1:
            self.adapter_attention = HubertAttention(
                embed_dim=config.hidden_size,
                num_heads=config.num_adapter_attention_heads,
                dropout=config.attention_dropout,
                is_decoder=False,
                ratio=config.ratio,
            )
    ...
    def forward(self, hidden_states, attention_mask=None, output_attentions=False):
        ...
        hidden_states, self_attn_weights, _ = self.attention(
            hidden_states, attention_mask=attention_mask, output_attentions=output_attentions
        )
        intermediate_states = self.dropout(hidden_states) 

        hidden_states = attn_residual + intermediate_states        
        hidden_states = self.layer_norm(hidden_states)
        intermediate_states_feedforward = self.feed_forward(hidden_states)

        if self.adapter_layer_index == 1:
            adapter_out, adapter_attn_weights, _ = self.adapter_attention(hidden_states=intermediate_states_feedforward) 
            intermediate_states_feedforward = self.dropout(adapter_out) + intermediate_states_feedforward 

        hidden_states = hidden_states + intermediate_states_feedforward
        ...
        if output_attentions:
            ...
            if self.adapter_layer_index == 1:
                outputs += (self_attn_weights, adapter_attn_weights)
...
class HubertAttnAdapterLayer(nn.Module):
    def __init__(self, config):
        ...
        """
        super().__init__()
        self.input_dim = config.hidden_size // config.ratio
...
class HubertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout)
        self.layers = nn.ModuleList([HubertEncoderLayer(config, config.adapter_layer_index[i]) for i in range(config.num_hidden_layers)])
        self.gradient_checkpointing = False
    ...
    def forward(
        ...
        all_adapter_attentions = () if output_attentions else None
        ...
        for i_layer, layer in enumerate(self.layers):
            ...
            if output_attentions:
                ...
                if layer.adapter_layer_index == 1:
                    all_adapter_attentions = all_adapter_attentions + (layer_outputs[2], )
        ...
        ), all_self_attentions, all_adapter_attentions]
...
class HubertModel(HubertPreTrainedModel):
    def __init__(self, config: HubertConfig):
        ...
        self.mert_output_step_mlp = nn.Sequential(
            nn.Linear(151,150//2),
            nn.GELU(),
            nn.Linear(150//2,1),
            nn.Dropout(0.1)
    ...
    def forward(
        ...
        hidden_states = self.feature_projection(extract_features) #atch 151 768
        ...
        encoder_outputs, all_self_attentions, all_adapter_attentions = encoder_out[0], encoder_out[1], encoder_out[2]
        
        encoder_outputs = torch.stack(encoder_outputs.hidden_states) 

        audio_feature = self.mert_output_step_mlp(encoder_outputs[-1].permute(0,2,1)).squeeze(-1) #batch 768
        audio_feature = self.audio_feature_mlp(audio_feature) #batch 82 
        

        return [audio_feature, all_self_attentions, all_adapter_attentions]
